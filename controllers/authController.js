...
const controller = {
    login: async (req, res) => {
        try {
            ...
            if (!user) {
                return res.status(404).json({ result: false, error: 'L\'adresse mail ne correspond à aucun compte' });
            }

            const blockEndTime = new Date(user?.lastAuthFailedAttempt?.getTime() + config.api.authBlockDurationInMinutes * 60000);
            const remainingBlockTimeInMinutes = Math.ceil((blockEndTime - new Date().getTime()) / 60000);

            if (user.consecutiveAuthFailedAttempts >= config.api.authAttemptsBeforeBlock && blockEndTime > new Date()) {
                return res.status(401).json({ result: false, error: `${translations.tooManyAuthFail} ${utils.language.plural('minute', remainingBlockTimeInMinutes)}` });
            }
    ...
    register: async (req, res) => {
        try {
            const { email, password, lastName, firstName, companyId } = req.body;

            if (!email || !password || !lastName || !firstName || !companyId) {
            ...
            await models.user.create({ email, password, lastName, firstName, companyId });
    ...
    refreshToken: async (req, res) => {
        try {
            ...
            if (!refreshToken) {
                return res.status(400).json({ result: false, error: 'Refresh token requis' });
            } else {
                jwt.verify(refreshToken, config.jwt.refreshSecret, async (err, decoded) => {
                    if (err) {
                        return res.status(401).json({ result: false, error: 'Refresh token invalide' });
                    }
